...
function HabitDetailPanel({ step, routine, checkins, selectedDate }: HabitDetailPanelProps) {
  const getLast7CompletionTimes = useRoutine2Store((s) => s.getLast7CompletionTimes)
  const toggleStep = useRoutine2Store((s) => s.toggleStep)
...
export function RoutinesView({ checklistId: _checklistId }: RoutinesViewProps) {
  ...
  const { routines, checkins, loading, activeTimer, loadRoutines, toggleStep, markStepFailed, markAllPendingFailed, resetStep, startQueue, startTimer, getTodayCheckin } = useRoutine2Store()
  const { deleteRoutineDef } = useRoutineSystem()
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bgSecondary }}>
      ...
      {/* FAB */}
      <Pressable
        onPress={() => setShowGlobalEdit(true)}
        style={{
